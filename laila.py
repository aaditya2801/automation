...
print("--------------------------------".center(125))                    
engine.runAndWait()
p.speak("I am your voice assistant laila")
...
while True:
  ...
  if("hello" in ch) or ("hi" in ch) and ("heya" in ch) or ("hey" in ch):
    p.speak("hello addy , how are you ...?")
    print()
    input("\n Hit enter to continue...")
  elif("fine" in ch) or ("what" in ch) and ("about" in ch):
    p.speak("i am fine too")
  elif("tell" in ch) or ("about" in ch) and ("yourself" in ch):
    p.speak("i am your voice assistant laila, i am very intelligent , and i can intregrate multiple technologies for you")
    ...
    p.speak("such as create key-pair")
    print()
    print(" * Create key-pair".center(126))
    ...
    p.speak("run Salary Predictor Code on the top of EC-2 Instance")
    print()
    print(" * Run Salary Predictor Code on the top of EC-2 Instance".center(126))
  ...
  elif("key" in ch) or ("pairs" in ch) or ("key pairs" in ch) or ("players" in ch):
    ...
    ip = input("Enter Instance IP : ")
    os.system("ssh root@{} ansible-playbook bash-script/code.yml ; rpm -q gedit ; rpm -q httpd ; rpm -q mariadb ; rpm -q tcpdump ; pip3 list".format(ip))
    ...
    ip = input("Enter Instance IP : ")
    os.system("ssh root@{} yum install nfs-utils ; systemctl start nfs-server ; systemctl status nfs-server ; rpm -q nfs-utils".format(ip))
    p.speak("nfs server is successfully configured")
    input()
  elif("ntp" in ch) or ("client" in ch) or ("ntp client" in ch):
    os.system("ssh root@{} yum install chrony ; systemctl start chronyd ; systemctl status chronyd ; rpm -q chrony ; chronyc sources".format(ip))
    p.speak("ntp is successfully configured")
    input()
  elif("user" in ch) or ("tenant" in ch):
    p.speak("Enter User Name")
    uname = input("Enter User Name : ")
    os.system("ssh root@{} useradd {} ; passwd {} ; cat /etc/passwd".format(ip,uname,uname))
